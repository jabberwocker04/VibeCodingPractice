...
class LiveBotCommandHandler:
    ...
    지원 명령어:
      ...
      /params               현재 파라미터 조회
      /params key=val ...   파라미터 수정
      /config               현재 설정 조회
      /config key=val ...   설정 수정 (quantity, max_position_qty, tick_seconds)
    """
    ...
    def _status_message(self) -> str:
        ...
        return (
            ...
            f"자산: ${float(str(s.get('equity', 0))):,.2f} | 손익: {pnl_sign}${float(str(s.get('pnl', 0))):,.2f} ({pnl_sign}{float(str(s.get('pnl_pct', 0))):.2f}%)\n"
            f"현금: ${float(str(s.get('cash', 0))):,.2f} | 보유: {s.get('position_qty')}주 | 거래: {s.get('trades')}회"
        )
