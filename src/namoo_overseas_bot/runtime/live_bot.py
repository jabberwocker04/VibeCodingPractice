...
class LiveTradingBot:
    ...
    def _warmup_strategy(self) -> None:
        try:
            candles = fetch_candles(self.symbol, interval=self.interval, period=self.history_period)
    ...
    def _process_tick(self) -> None:
        ...
        if signal == Signal.BUY:
            ...
            if current_pos + self.quantity <= self.max_position_qty:
                try:
                    ...
                    fill_message = f"[체결] 매수 {fill.qty}주 {fill.symbol} @ ${fill.price:,.2f}"
                    with self._lock:
                        self._status.trades += 1
                        self._trade_history.append(
                            {"ts": now, "side": "buy", "price": price, "qty": self.quantity, "symbol": self.symbol}
                        )
                except ValueError as e:
                    fill_message = f"[건너뜀] 매수 실패: {e}"
            else:
                fill_message = (
                    f"[건너뜀] 최대 보유 수량 초과 (현재={current_pos}, 최대={self.max_position_qty})"
                )
            ...
            if current_pos >= self.quantity:
                try:
                    ...
                    fill_message = f"[체결] 매도 {fill.qty}주 {fill.symbol} @ ${fill.price:,.2f}"
                    with self._lock:
                        self._status.trades += 1
                        self._trade_history.append(
                            {"ts": now, "side": "sell", "price": price, "qty": self.quantity, "symbol": self.symbol}
                        )
                except ValueError as e:
        ...
        with self._lock:
            ...
            self._price_history.append(
                {"ts": now, "price": price, "equity": equity, "pnl": pnl, "signal": signal.value}
            )
    ...
    def _signal_to_korean(signal: Signal) -> str:
        return {Signal.BUY: "매수", Signal.SELL: "매도", Signal.HOLD: "대기"}.get(signal, "대기")
    ...
    def swap_strategy(self, new_strategy: BaseStrategy) -> None:
        ...
        threading.Thread(target=self._warmup_strategy, name="strategy-warmup", daemon=True).start()
        self._safe_notify(
    ...
    def update_strategy_params(self, **kwargs: object) -> None:
        with self._lock:
            ...
            self._status.last_signal = Signal.HOLD.value
        threading.Thread(target=self._warmup_strategy, name="param-warmup", daemon=True).start()
        self._safe_notify(
    ...
    def get_strategy_info(self) -> dict[str, object]:
        with self._lock:
            return {"name": self.strategy.name, "params": self.strategy.get_params()}
